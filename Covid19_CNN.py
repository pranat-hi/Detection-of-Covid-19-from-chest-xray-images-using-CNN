...
main = tkinter.Tk()
main.title("Detection of COVID-19 from Chest X-Ray Images Using Convolutional Neural Networks")
...
def buildCNNModel():
    ...
    if os.path.exists('model/model.json'):
        ...
        accuracy = acc[9] * 100
        text.insert(END,"Chest X-Ray Based Covid-19 Prediction Accuracy : "+str(accuracy))
    else:
...
font = ('times', 15, 'bold')
title = Label(main, text='Detection of COVID-19 from Chest X-Ray Images Using Convolutional Neural Networks ', justify=LEFT)
...
contextButton = Button(main, text="Build CNN Covid-19 Model", command=buildCNNModel)
...
predictButton.place(x=710,y=100)
